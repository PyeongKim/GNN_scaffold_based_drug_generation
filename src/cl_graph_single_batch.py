...
class ScaffoldGNN(nn.Module):
    def __init__(self, num_auxiliary, num_prop_rounds=[3,3,2,2,2,2],
                 num_node_features=[8,4,3,2], num_edge_features=[4,5],
        ...
        self.init_edges_mlp1 = nn.Embedding(self.num_edge_types - 1, node_emb_dim)
    ...
    def whole_graph_encode(self, g_whole, conditions):
        """
        ...
        g_whole = self.whole_graph_prop(g_whole, conditions)
    ...
    def scaffold_propagate(self, g_scaffold, conditions):
        ...
        g_scaffold = self.scaffold_graph_prop(g_scaffold, conditions)

        return g_scaffold

    def add_nodes_encode(self, g_scaffold, conditions, training=True):
        g_scaffold = self.add_nodes_prop(g_scaffold, conditions)
        readout = self.add_nodes_readout(g_scaffold)
        readout = torch.cat([readout, conditions])
    ...
    def init_nodes_encode(self, g_scaffold, categorical_node_feat):
        ...
        readout = torch.cat([readout, node_feat])
        node_feat = self.init_nodes_mlp2(readout)
        return node_feat

    def add_edges_encode(self, g_scaffold, conditions, training=True):
        g_scaffold = self.add_edges_prop(g_scaffold, conditions)
    ...
    def init_edges_encode(self, g_scaffold, categorical_edge_feat):
        ...
        readout = torch.cat([readout, edge_feat])
        edge_feat = self.init_edges_mlp2(readout)
        return edge_feat

    def select_nodes_encode(self, g_scaffold, conditions):
        src = g_scaffold.number_of_nodes() - 1
        possible_dest = range(src)
        g_scaffold = self.select_nodes_prop(g_scaffold, conditions)
    ...
    def forward(self, g_whole, g_scaffold, conditions, actions):
        self.prepare_for_traininig()
        z, logvar, mu = self.whole_graph_encode(g_whole, conditions)
        g_scaffold = self.scaffold_propagate(g_scaffold, conditions)
        # print("after scaffold:",g_scaffold.edata["edge_features"].size())
        # concat conditions with latent vector for decoding process
        conditions = torch.cat([z, conditions])
        # print("conditions", conditions.size())
        add_node = True
        index = 0
        while add_node:
            # add node but which node?
            node_dist = self.add_nodes_encode(g_scaffold, conditions)  # if stop add_node = False (setting from dataset)
            self.node_probability.append(node_dist)
            if actions[index][0] == self.num_node_types:
                add_node = False
                continue
            else:
                g_scaffold.add_nodes(1)
                num_nodes = g_scaffold.number_of_nodes()
                # get node feature
                init_node = actions[index][0]
                new_node_feat = self.init_nodes_encode(g_scaffold, init_node)
                # print(new_node_feat.size(), g_scaffold.nodes[num_nodes - 1].data['node_features'].size())
                g_scaffold.nodes[num_nodes - 1].data['node_features'] = new_node_feat.unsqueeze(0)
            edge_index = 0
            add_edge = True
            #print(actions[index], actions)
            while add_edge and edge_index < len(actions[index][1]):
                edge_add_action = self.add_edges_encode(g_scaffold, conditions)
                self.edge_probability.append(edge_add_action)
                # print("%%%%%%%%%%%",actions[index], edge_index)
                if actions[index][1][edge_index] == self.num_edge_types - 1:
                    add_edge = False
                    continue
                else:
                    init_edge = actions[index][1][edge_index]  # edge types
                    # print("after scaffold:",g_scaffold.edata["edge_features"].size())
                    select_node = self.select_nodes_encode(g_scaffold, conditions)
                    # print("###############after scaffold:",g_scaffold.edata["edge_features"].size())
                    self.dest_probability.append(select_node)
                    src, dest = num_nodes - 1, actions[index][2][edge_index]  # edge dest
                    src_list = [src, dest]
                    dest_list = [dest, src]
                    g_scaffold.add_edges(src_list, dest_list)
                    edge_emb = self.init_edges_encode(g_scaffold, init_edge)
                    # print("###edge###",g_scaffold.edges[src_list, dest_list].data['edge_features'].size(),torch.stack([edge_emb, edge_emb], dim=0).size())
                    g_scaffold.edges[src_list, dest_list].data['edge_features'] = torch.stack([edge_emb, edge_emb], dim=0)
                edge_index += 1
            index += 1

        return z, logvar, mu, self.node_probability, self.edge_probability, self.dest_probability



    def sample(self):
        """
        # TO DO: implement sampling for validation
        """
        return None
...
class ConditionalProp(nn.Module):
    ...
    def forward(self, g, auxiliary):
        ...
        for round in range(self.num_prop_rounds):
            g = self.prop_layers[round](g, auxiliary)
...
class GraphReadOut(nn.Module):
    ...
    def forward(self, g):
        ...
        if graph_feats.size(0) == 1:
            return graph_feats.squeeze(0)  # if batch == 1 delete batch dimension
        else:
            return graph_feats
